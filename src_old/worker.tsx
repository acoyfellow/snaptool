...
import { compileWithBun } from "./utils/compiler";
...
app.post("/test-compile", async (c) => {
  try {
    if (!c.env.BUN_COMPILER_DO) {
      return Response.json({ error: "BUN_COMPILER_DO not available" }, { status: 500 });
    }
    const container = c.env.BUN_COMPILER_DO.get(c.env.BUN_COMPILER_DO.idFromName('compiler'));
    const response = await container.fetch('http://container/compile', {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ code: 'export default { fetch() { return new Response("hello"); } }' })
    });
    const result = await response.text();
    ...
    console.error("Container compile test error:", error);
    return Response.json({ error: error.message }, { status: 500 });
  }
});
...
export class BunCompiler extends DurableObject<Cloudflare.Env> {
  ...
  constructor(ctx: DurableObjectState, env: Cloudflare.Env) {
    ...
    console.log("BunCompiler constructor called");
  ...
  async fetch(req: Request): Promise<Response> {
    try {
      ...
      console.log("Fetching from container on port 3000");
